...
class Corpus():
    ...
    def concorde(self, word, lengthContext):
        ...
        for i in list:
            ...
            for j in range(0, len(x)-1):
                ...
                after = " ".join(after)
                listConcord = listConcord.append({'contexte gauche': str(before), 'motif trouvé': str(
                    word), 'contexte droit': str(after)}, ignore_index=(True))
        return listConcord
    ...
    def Evolution_mot(self, w):
        # Contient les numéros des docs qui contiennent le mot clé
        all_docs = self.search(w)
        ...
        if len(all_docs) != 0:  # Teste si le mot a été trouvé dans le corpus, si oui exécution du code ci-dessous
            ...
            for i in all_docs:  # parcours de tous les docs
                # Liste contenant toutes les dates des documents qui contiennent le mot clé
                liste_date.append(self.get_doc(i).get_date())
                liste_occ.append(self.nettoyer_texte(self.get_doc(i).get_text()).count(
                    self.nettoyer_texte(w)))  # Liste contenant les occurences du mot clé dans les documents

            # Ajout des dates dans le dictionnaire
            dico_date["Dates"] = liste_date
            # Ajout des occurences dans le dictionnaire
            dico_date["Occurence"] = liste_occ

            # Création du DataFrame à partir du dictionnaire dico_date
            df = pandas.DataFrame.from_dict(dico_date)
            # Trier le dataframe par date
            df.sort_values(by=['Dates'], inplace=True, ascending=True)
            # Transfomer les occurences en somme cumulée
            df['Occurence'] = numpy.cumsum(df['Occurence'])
            return df
